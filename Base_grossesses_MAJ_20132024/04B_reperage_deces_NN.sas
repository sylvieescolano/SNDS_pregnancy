...
/*Identifiants BNA1 des nouveaux nés + passage ORAUSER pour jointure*/
PROC SQL;
   CREATE TABLE orauser.CHAINAGEME_ALL AS 
   SELECT DISTINCT t1.BEN_NIR_ANO1, T1.ISSUE,t1.BNA_OK
...
PROC SQL;
   ...
   SELECT DISTINCT t1.BEN_NIR_ANO1 AS BEN_NIR_ANO, 
          t2.BEN_NIR_PSA
      FROM ORAUSER.CHAINAGEME_ALL t1
           LEFT JOIN ORAVUE.IR_BEN_R t2 ON (t1.BEN_NIR_ANO1 = t2.BEN_NIR_ANO)
...
ou dans les 28j
+ retirer la date de deces si <dat_evt (21 dates)*/

...
Creation base 1 : Tous les séjours d'EOPE après filtrage , dates entre '1Jan2013'd AND '31Dec2024'
(modifiable pour actualiser la base)*/
...
/* POUR FILTRAGE : Liste des finess geographiques APHP, APHM et HCL a  supprimer pour eviter les doublons */
...
/****** CODES CIM-10 A RECHERCHER EN DP (table B et UM) ******/
...
/****** CODES CIM-10 A RECHERCHER EN DAS (table D) ******/
...
/*definition des variables pour années aaaa ou sous tables aa_xx*/
/*(Pour integrer les derniers lots PMSI incomplets, ajouter le nom du lot pour la dernière année)*/

%if &aaaa.=2024 %then %let aa=24/*_10*/;
...
/*Compter le nombre de sejours par issue dans chaque extraction*/
...
/*agreger toutes les tables 2012-2024*/
data work.PMSI_20122024;
	set REP.sejg_:;
run;
...
PROC SQL;
   CREATE TABLE WORK.PMSI_2013_2024 AS 
   SELECT t1.*,
          /* annee */
            (YEAR(t1.dat_evt)) AS annee
      FROM WORK.PMSI_20122024 t1
      WHERE t1.dat_evt BETWEEN '1Jan2013'd AND '31Dec2024'd
      ORDER BY t1.dat_evt;
QUIT;
...
PROC SQL;
   ...
   SELECT t1.indsej, 
      ...
      FROM WORK.PMSI_2013_2024 t1
      GROUP BY t1.indsej
      HAVING (CALCULATED COUNT_of_indsej) > 1;
QUIT;
/*Variable COD_SEX*/
proc freq data=WORK.PMSI_2013_2024;
tables cod_sex ; run;


/*retrait doublons, COD_SEX=1 ET ENREGISTREMENT DANS REP*/
PROC SQL;
   ...
   SELECT t1.*
	  ...
	  WHERE t2.indsej IS NULL and COD_SEX='2'
      ;
QUIT;
...
PROC SQL;
 CREATE TABLE WORK.COUNT_SEJG_base1 AS 
   SELECT t1.issue, 

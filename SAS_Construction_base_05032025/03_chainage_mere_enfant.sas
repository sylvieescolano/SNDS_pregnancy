...
Puis chainage avec IR BEN R (attention la table évolue, sauvegarde du 02/02/2024 
dans repmedgr)
...
%INCLUDE '/home/sas/42a001279410899/sasdata/REPMEDGR/Base_Grossesse/Construction_base/macros.sas';
...
/*Reunir tous les séjours*/


data work.sej_N;
	set WORK.SEJN_2: ;
run; /* ajouter verification de l'absence de doublons*/
...
/*ATTENTION Pour éviter modifications liées aux MAJ de IRBENR:
	utiliser sauvegarde dans REPMEDGR : "IR_BEN_R_02022024"*/
...
PROC SQL;
   ...
   SELECT t1.BEN_CDI_NIR, 
		  ...
		  t2.issue,
		('1') as IRBENR_OK
      FROM repmedgr.IR_BEN_R_02022024 t1 
...
PROC SQL;
   CREATE TABLE WORK.chainage8 AS 
   SELECT t1.classNN, 
          t1.indsej, 
          t1.indsej1, 
          t1.NIR_ANO_171, 
   ...
   SELECT DISTINCT t1.indsej, 
          ...
          /* BEN_NIR_ANO1 */
            (CASE 
                ...
                ELSE ''
            END) AS BEN_NIR_ANO1
      FROM WORK.chainage8 t1
           LEFT JOIN REPMEDGR.IR_BEN_R_02022024 t2 ON (t1.NIR_ANO_171 = t2.BEN_NIR_PSA) AND (t1.COD_SEX1 = 
          t2.BEN_SEX_COD);
QUIT;

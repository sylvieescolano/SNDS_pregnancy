...
PROC SQL;
   CREATE TABLE WORK.BNA_SEJG_PMSI AS 
   SELECT DISTINCT t2.BEN_NIR_ANO
      FROM rep.base1 t1
           INNER JOIN repMEDGR.ir_ben_r_02022024 t2 ON (t1.NIR_ANO_17 = t2.BEN_NIR_PSA)
      WHERE t2.BEN_NIR_ANO NOT IS MISSING;
QUIT;

/*Récupérer tous les PSA de IRBENR associés à ces BNA*/
PROC SQL;
   ...
   SELECT DISTINCT t1.BEN_CDI_NIR, 
      ...
      FROM repMEDGR.IR_BEN_R_02022024 t1 
           INNER JOIN WORK.BNA_SEJG_PMSI t2 ON (t1.BEN_NIR_ANO = t2.BEN_NIR_ANO)
...
/*(maximum 1 an de différence = on garde AGE_ANN (PMSI)*/
